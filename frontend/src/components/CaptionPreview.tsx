...
import { getCaptionPositionClass, getCaptionPreviewWords, getCaptionPresetLabel } from '../utils/captionDesigner';

export default function CaptionPreview({ style }: { style: CaptionStyle }) {
  const words = getCaptionPreviewWords(style);
  const activeIndex = style.preset === 'karaoke' ? Math.min(1, words.length - 1) : -1;
