...
assert.match(panelSource, /<ClipWorkspaceHeader/);
...
assert.match(clipExportFilesSource, /export function buildClipBatchManifest\(/);
assert.match(clipExportFilesSource, /export async function writeClipBatchManifest\(/);
assert.doesNotMatch(clipExportFilesSource, /useAIStore|useEditorStore|fetch\(|localStorage|window\.electronAPI/, 'clip export file service must receive side effects as dependencies');
assert.doesNotMatch(reviewActionsSource, /useAIStore|useEditorStore|fetch\(|localStorage/, 'clip review actions must receive workspace mutations as dependencies');
assert.match(reviewActionsSource, /isSameClipRange\(draft, clip\)/);
...
assert.match(headerSource, /Review \$\{pendingReviewCount\}/);
assert.match(headerSource, /Prepare \$\{queueSummary\.prepare\}/);
assert.match(headerSource, /label="Exporting"/);
assert.match(headerSource, /label="Retry"/);
assert.match(headerSource, /label="Failed"/);
assert.match(headerSource, /label="Exported"/);
assert.match(panelSource, /removeMatchingClipSuggestions/);
assert.match(reviewActionsSource, /createClipDraft\(clip, 'ai', undefined, false\)/);
