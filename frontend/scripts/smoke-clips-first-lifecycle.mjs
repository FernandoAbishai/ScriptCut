...
assert.match(panelSource, /type AIJobRunContext = \{[\s\S]*workspaceEpoch: number/);
assert.match(panelSource, /const activeAIJobRunRef = useRef<AIJobRunContext \| null>\(null\)/);
assert.match(panelSource, /Another AI action is still running/);
assert.match(panelSource, /const isCurrentAIJobRun = useCallback/);
assert.match(panelSource, /let jobRes: Response;[\s\S]*jobRes = await fetch\(`\$\{backendUrl\}\/jobs\/\$\{jobId\}`\)[\s\S]*Connection interrupted; checking AI job status[\s\S]*continue;/);
assert.match(panelSource, /if \(!jobRes\.ok\) \{[\s\S]*jobRes\.status === 404[\s\S]*AI job status unavailable; retrying[\s\S]*continue;/);
assert.match(panelSource, /let job: AIJob<T>;[\s\S]*job = \(await jobRes\.json\(\)\) as AIJob<T>[\s\S]*AI job status was unreadable; retrying[\s\S]*continue;/);
assert.match(panelSource, /setActiveAIJob\(\{ \.\.\.job, \.\.\.context, workspaceEpoch: run\.workspaceEpoch, runId: run\.id \}\)/);
assert.match(panelSource, /workspaceEpoch: run\.workspaceEpoch/);
assert.match(panelSource, /runId: run\.id/);
assert.match(panelSource, /current && current\.workspaceEpoch !== clipWorkspaceEpoch \? null : current/);
const cancelAIJobBody = panelSource.match(/const cancelAIJob = useCallback\(async \(\) => \{([\s\S]*?)\n\s*\}, \[/)?.[1] || '';
...
const retryAIJobBody = panelSource.match(/const retryAIJob = useCallback\(async \(\) => \{([\s\S]*?)\n\s*\}, \[/)?.[1] || '';
assert.match(retryAIJobBody, /const sourceJob = activeAIJob/);
assert.match(retryAIJobBody, /sourceJob\.workspaceEpoch !== useAIStore\.getState\(\)\.clipWorkspaceEpoch/);
assert.match(retryAIJobBody, /run = beginAIJobRun\(\)/);
assert.match(retryAIJobBody, /pollAIJob<unknown>\(jobId, sourceJob\.label, context, run\)/);
assert.match(retryAIJobBody, /if \(!isCurrentAIJobRun\(run\)\) return/);
