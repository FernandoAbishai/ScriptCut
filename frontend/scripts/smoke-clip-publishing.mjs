...
assert.match(cardSource, /aiBusy: boolean/);
assert.match(cardSource, /disabled=\{isSuggested \|\| aiBusy \|\| isGeneratingPublishingCopy\}/);
assert.match(panelSource, /aiBusy=\{isProcessing\}/);
assert.match(cardSource, /onChange\(\{ title: suggestion \}\)/);
assert.match(panelSource, /function getClipMetadataInputFingerprint\(draft: ClipDraft, words: Word\[\]\)/);
assert.match(panelSource, /inputFingerprint = getClipMetadataInputFingerprint\(draft, words\)/);
assert.match(panelSource, /useAIStore\.getState\(\)\.clipDrafts\.find\(\(item\) => item\.id === draft\.id\)/);
assert.match(panelSource, /useEditorStore\.getState\(\)\.words/);
assert.match(panelSource, /getClipMetadataInputFingerprint\(currentDraft, currentWords\) !== inputFingerprint/);
assert.match(panelSource, /mergeGeneratedPublishingCopy\(currentDraft, data\)/);
assert.match(panelSource, /sourceJob\.inputFingerprint/);
assert.match(panelSource, /getClipMetadataInputFingerprint\(currentDraft, currentWords\) !== sourceJob\.inputFingerprint/);
assert.match(panelSource, /mergeGeneratedPublishingCopy\(currentDraft, metadata\)/);
